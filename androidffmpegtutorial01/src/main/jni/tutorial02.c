...
int naSetCommand(JNIEnv *pEnv, jobject pObj, jstring pCommand) {
    ffCommand = (char *)(*pEnv)->GetStringUTFChars(pEnv, pCommand, NULL);

    if(NULL == ffCommand) {
        LOGI("LEI    command is NULL");
    }else {
        LOGE("LEI    command is : %s",ffCommand);
    }
    return system(ffCommand);
}
...
jobject createBitmap(JNIEnv *pEnv, int pWidth, int pHeight) {
	...
	//create the bitmap
	return (*pEnv)->CallStaticObjectMethod(pEnv, javaBitmapClass,
	                         mid, pWidth, pHeight, javaBitmapConfig);
}

jintArray naGetVideoRes(JNIEnv *pEnv, jobject pObj) {
    jintArray lRes;
    fprintf(stdout, "LEI    enter naGetVideoRes\n");
	if (NULL == codecCtx) {
	    fprintf(stderr, "LEI    codecCtx == NULL\n");
		return NULL;
	}
...
jint JNI_OnLoad(JavaVM* pVm, void* reserved) {
	...
	nm[6].name = "naSetCommand";
	nm[6].signature = "(Ljava/lang/String;)I";
	nm[6].fnPtr = (int*)naSetCommand;
